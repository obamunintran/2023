...
#        xml_file_link = self.env.user.company_id.factura_dir + '/' + invoice_obj.number.replace('/', '_') + '.xml'
#        xml_file = open(xml_file_link, 'w')
#        xml_invoice = base64.b64decode(file_content)
...
#                raise Warning("Factura ya existente con la referencia del vendedor %s"%(customer_reference))
    ...
    def get_or_create_product(self, default_code, product_name, clave_unidad, unit_price, clave_producto, sale_ok=True, purchase_ok=False):
        ...
        if not product_exist:
            ...
            product_vals = {'default_code':default_code, 'name':product_name, 'standard_price' : unit_price,'unidad_medida' : um_descripcion, 'clave_producto' : clave_producto, }
